...
c = a + b
list_of_a_value = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
with tf.Session() as sess:
    writer = tf.summary.FileWriter('./my_graph', sess.graph)
    for a_value in list_of_a_value:
        print(sess.run(c, feed_dict={a: a_value}))

x = tf.constant([[1, 2, 3], [2, 3, 4]])
print(tf.Graph.is_feedable(tf.get_default_graph(), x))
...
with tf.Session() as sess:
    replace_dict = {x: 15}
    print(sess.run(y, feed_dict=replace_dict))
